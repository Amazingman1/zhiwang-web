...
export function paseToNodeTree(_data, fields, openIds) {
  ...
  循环退出的条件是，分组列表里的每一项在当前的路径记录都没有对应的父节点记录*/
    do {
      usedNode = []
      for (const prop in vNodeGroup) {
        // 如果
        if (Object.prototype.hasOwnProperty.call(path_store, prop)) {
  ...
  try {
    ...
    path.forEach(function(item, index) {
      if (index === 0) {
        ...
        if (Object.prototype.hasOwnProperty.call(tem, item)) {
          tem = tem[item]
        }
      } else {
        tem = tem.children[item]
...
export function loadingShow(list, fn) {
  return this.$loading({
    lock: true,
    text: 'Loading',
...
export function loadingHide(list, fn) {
  return this.$loading({
    lock: true,
    text: 'Loading',
    spinner: 'el-icon-loading',
